...
// Standard paths (DATA_DIR imported from paths.ts)
...
export async function updateCursorContextForProject(projectName: string, port: number): Promise<void> {
  ...
  try {
    ...
    const response = await fetch(
      `${formatWorkerUrl(port)}/api/context/inject?project=${encodeURIComponent(projectName)}`
    );
...
async function setupProjectContext(targetDir: string, workspaceRoot: string): Promise<void> {
  ...
  try {
    ...
    const healthResponse = await fetch(`${formatWorkerUrl(port)}/api/readiness`);
