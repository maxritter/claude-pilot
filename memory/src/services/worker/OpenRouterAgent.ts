 ...
 *
 * Alternative to SDKAgent that uses OpenRouter's official SDK
 * for accessing 300+ models from different providers.
 *
 * Responsibility:
 * - Call OpenRouter API via official SDK for observation extraction
...
// OpenAI-compatible message format (used by SDK)
...
export class OpenRouterAgent {
  ...
  async startSession(session: ActiveSession, worker?: WorkerRef): Promise<void> {
    try {
      ...
      const initResponse = await this.queryOpenRouterMultiTurn(session.conversationHistory, client, model);
      ...
      for await (const message of this.sessionManager.getMessageIterator(session.sessionDbId)) {
        ...
        if (message.type === 'observation') {
          ...
          const obsResponse = await this.queryOpenRouterMultiTurn(session.conversationHistory, client, model);
          ...
          const summaryResponse = await this.queryOpenRouterMultiTurn(session.conversationHistory, client, model);
  ...
  /**
   * Convert shared ConversationMessage array to SDK-compatible message format
   */
  private conversationToMessages(history: ConversationMessage[]): ChatMessage[] {
  ...
  /**
   * Query OpenRouter via official SDK with full conversation history (multi-turn)
  ...
  private async queryOpenRouterMultiTurn(
    ...
    const messages = this.conversationToMessages(truncatedHistory);

    // Validate we have at least one message
    if (messages.length === 0) {
      throw new Error('Cannot query OpenRouter: conversation history is empty after truncation');
    }

    const totalChars = truncatedHistory.reduce((sum, m) => sum + m.content.length, 0);
    const estimatedTokens = this.estimateTokens(truncatedHistory.map(m => m.content).join(''));

    logger.debug('SDK', `Querying OpenRouter SDK (${model})`, {
      turns: truncatedHistory.length,
      totalChars,
      estimatedTokens
    });

    // Use SDK for API call - SDK uses unwrapAsync which:
    // - Returns ChatResponse directly on success
    // - Throws on error (no Result wrapper)
    let response;
    try {
      ...
      // SDK threw an exception - extract meaningful error message
      let errorMessage: string;
      if (sdkError instanceof Error) {
        errorMessage = sdkError.message;
      } else if (typeof sdkError === 'object' && sdkError !== null) {
        errorMessage = (sdkError as any).message || (sdkError as any).error?.message || JSON.stringify(sdkError);
      } else {
        errorMessage = String(sdkError ?? 'Unknown error');
      }

      logger.error('SDK', 'OpenRouter SDK error', {
        errorType: sdkError?.constructor?.name,
        message: errorMessage
      });
      throw new Error(`OpenRouter SDK error: ${errorMessage}`);
    }

    if (!response.choices?.[0]?.message?.content) {
      logger.error('SDK', 'Empty response from OpenRouter SDK');
      return { content: '' };
    }
    ...
    if (tokensUsed) {
      ...
      logger.info('SDK', 'OpenRouter SDK usage', {
        model,
        inputTokens,
        outputTokens,
   ...
   */
  private getOpenRouterConfig(): { apiKey: string; model: string } {
