...
export class SettingsRoutes extends BaseRouteHandler {
  ...
  /**
   * GET /api/providers/mistral/models - Fetch available Mistral models
    ...
    if (!apiKey) {
      ...
      res.json({
        models: [
          ...
          { id: 'open-mistral-nemo', name: 'Open Mistral Nemo (open-source)' },
          { id: 'codestral-latest', name: 'Codestral (code-focused)' },
          { id: 'devstral-small-latest', name: 'Devstral Small (development)' },
        ],
        source: 'defaults'
      });
      return;
