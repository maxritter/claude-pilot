...
export function ContextSettingsModal({
  ...
  // Fetch Mistral models when provider is mistral
  useEffect(() => {
    if (formState.CLAUDE_MEM_PROVIDER === 'mistral' && isOpen) {
      setMistralModelsLoading(true);
      fetch(`${API_ENDPOINTS.BASE}/providers/mistral/models`)
        .then(res => res.json())
        .then(data => {
  ...
  }, [formState.CLAUDE_MEM_PROVIDER, isOpen, formState.CLAUDE_MEM_MISTRAL_API_KEY]);
        ...
        <div className="modal-body">
          ...
          <div className="settings-column">
            ...
            >
              <FormField
                label="AI Provider"
                tooltip="Choose between Claude (via Agent SDK), Gemini, Mistral, or OpenRouter"
              >
                <select
                ...
                >
                  ...
                  <option value="mistral">Mistral (uses API key)</option>
              ...
              {formState.CLAUDE_MEM_PROVIDER === 'mistral' && (
                <>
                  <FormField
                    label="Mistral API Key"
                    tooltip="Your Mistral API key from console.mistral.ai (or set MISTRAL_API_KEY env var)"
                  >
                    <input
                      type="password"
                      value={formState.CLAUDE_MEM_MISTRAL_API_KEY || ''}
                      onChange={(e) => updateSetting('CLAUDE_MEM_MISTRAL_API_KEY', e.target.value)}
                      placeholder="Enter Mistral API key..."
                    />
                  </FormField>
                  <FormField
                    label="Mistral Model"
                    tooltip="Mistral model used for generating observations (live from API)"
                  >
                    <select
                      value={formState.CLAUDE_MEM_MISTRAL_MODEL || 'mistral-small-latest'}
                      onChange={(e) => updateSetting('CLAUDE_MEM_MISTRAL_MODEL', e.target.value)}
